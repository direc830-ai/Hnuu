...
const AdminNotifications = () => {
  ...
  const iconByType = useMemo(() => ({ security: ShieldAlert, alert: AlertTriangle, system: CheckCircle2, info: Info }), []);
  const colorByType = useMemo(() => ({ security: 'rose', alert: 'amber', system: 'emerald', info: 'blue' }), []);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoading(true);
      try {
        const [bookingsRes, lowBalRes] = await Promise.all([
        ...
        const live = [];
        (lowBalRes.data || []).forEach((p, i) => {
          live.push({ id: `low-${i}`, type: 'alert', title: 'Low Balance Alert', message: `${p.full_name || p.email} has ${p.balance_credits ?? 0} credits remaining.`, time: 'Live', read: false });
        });
        (bookingsRes.data || []).forEach((b, i) => {
          live.push({ id: `booking-${i}`, type: 'info', title: 'New Booking Recorded', message: `Seat ${b.seat_id} in ${b.floor} was booked (${b.status || 'confirmed'}).`, time: new Date(b.created_at).toLocaleString(), read: i > 1 });
        });
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button onClick={markAllRead} className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-all flex items-center gap-2">
            <Check className="w-4 h-4" /> Mark all as read
          </button>
          <button onClick={clearAll} className="px-5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold rounded-xl transition-all flex items-center gap-2">
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-4">
          {['all', 'unread', 'security'].map(f => (
            <button key={f} onClick={() => setActiveFilter(f)}
              ...
              {f === 'all' ? 'All Alerts' : f === 'unread' ? `Unread (${notifications.filter(n => !n.read).length})` : 'Security'}
            </button>
          ))}
        </div>
              ...
              <div key={notif.id} className={`p-6 flex gap-4 hover:bg-slate-50 transition-colors ${!notif.read ? 'bg-blue-50/30' : ''}`}>
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex gap-3">
                    {!notif.read && (
                      <button onClick={() => markRead(notif.id)} className="text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors">Mark as read</button>
                    )}
                    <button onClick={() => deleteOne(notif.id)} className="text-sm font-bold text-slate-400 hover:text-rose-600 transition-colors">Delete</button>
                  </div>
                </div>
              </div>
            );
