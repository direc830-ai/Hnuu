...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      ...
      const [studentsRes, activeRes, todayRes, lowBalRes, recentRes, allBookRes, allUserRes] = await Promise.all([
      ...
      setStats([
        { label: 'Total Students', value: String(studentsRes.count ?? 0), trend: 'From database', isUp: true, icon: Users, color: 'blue' },
        { label: 'Active Reservations', value: String(activeRes.count ?? 0), trend: 'Live now', isUp: true, icon: MapIcon, color: 'emerald' },
        { label: 'Today Bookings', value: String(todayRes.count ?? 0), trend: 'Since midnight', isUp: true, icon: TrendingUp, color: 'amber' },
        { label: 'Low Balance Alerts', value: String(lowBalRes.count ?? 0), trend: 'Under 5 credits', isUp: false, icon: ShieldCheck, color: 'purple' },
      ]);

      setRecentActivity((recentRes.data || []).map(b => ({
        title: `Seat ${b.seat_id} booked`,
        description: `${b.floor} · ${b.status}`,
        time: new Date(b.created_at).toLocaleString(),
      })));

      setAllBookings(allBookRes.data || []);
  ...
  const downloadReport = () => {
    ...
    lines.push('');
    lines.push('=== BOOKINGS ===');
    lines.push('ID,User ID,Seat,Floor,Start,End,Status,Created');
    ...
    a.href = url;
    a.download = `unispace-report-${Date.now()}.csv`;
